...
## 레이아웃 (모바일 세로) — 이슈 #5 CRT 그린+앰버 투톤

```
┌───────────────────────────┐
│ ▚▚  23:47:19 G.S.T.  ▮▮▯ │  장식 상태바 (앰버, aria-hidden)
├───────────────────────────┤
│ ▚ ≡≡ JOOPS ≡≡  [🌐 언어] │  앰버 보더 패널: 로고 + 스피드라인 워드마크
├───────────────────────────┤
│ ⌐ 지구 저궤도 // 실시간 ¬ │  코너 브래킷 프레임 (그린)
│     ◯ 지구본 + 궤도       │  Canvas: 지구 + 경위도 그리드
│    · · 줍스 100개 · ·     │        + 움직이는 줍스 점들
│ ⌐  100 개의 줍스가 궤도에¬ │
├───────────────────────────┤
│ ⌐수거한 쓰레기  [ 68.4% ]¬│  앰버 브래킷 + 앰버 진행바(인바 라벨)
│  [청소 완료▓▓▓▓▓░░░]      │  ▲25% ▲50% ▲75% 마일스톤 틱
│  12,847 조각              │  수치 = 앰버 위계
├───────────────────────────┤
│ 랭킹        주간 ▁▂▃      │  앰버 보더 터미널 테이블 + 스파크라인
│  1. Orbita  1,245 PTS ↑2 │  이름 fg(그린 위계) · 포인트 앰버
│  …                        │
│  (로그인 시) 나: 37위 ↑5   │  FR-1.8
├───────────────────────────┤
│   [ 초대코드로 시작하기 ]   │  미로그인 CTA (형광 그린) → 온보딩(M2)
└───────────────────────────┘
```

> **투톤 위계**(이슈 #5, [design-tokens](../../design/design-tokens.md)): 섹션 제목·수치·진행바·포인트는
> 앰버(`--color-secondary`), 본문·지구·이름·CTA는 그린/fg. 화면 전체에 `.crt-scanlines`
> 오버레이(게임 트리 body), 섹션 프레임은 `.crt-brackets`/`.panel-amber` 유틸 사용.
...
- 이 화면의 모든 문자열은 dictionary 키로. 초안 키:
  ...
  `firstScreen.orbitLabelTop`, `firstScreen.pointsUnit`,
  `firstScreen.cta.startWithInvite`, `common.appName`.
- 예외: 상태바의 `G.S.T.` 시계·배터리 글리프는 순수 장식(`aria-hidden`)이라 i18n 하지 않는다.
